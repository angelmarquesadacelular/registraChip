<?php
	...
	function availableBalance($empresa_id)
	{
		...
		if ($clientBalance->getError())
			echo "<br/><br/>Error al llamar el metodo balance<br/>".$clientBalance->getError();
	...
	function getOrder($empresa_id, $idProducto, $numero, $folioCliente)
	{
		...
		if ($client->getError())
			echo "<br/><br/>Error al llamar el metod get ordero<br/>".$client->getError();
		else
			return $respuesta;
	}
	...
	function doOrder($empresa_id, $idOrden, $folioCliente)
	{
		...
		if ($client->getError())
			echo "<br/><br/>Error al llamar el metodo do order<br/>".$client->getError();
		else
			return $respuesta;
	}
	...
	function checkOrder($empresa_id, $idOrden, $folioCliente)
	{
		...
		$wsdl = $datos[0];
		$client = new SoapClient($wsdl, true);
		$login = array('user' => $datos[1], 'password' => 
		$datos[2], 'operator' => $datos[3], 'topUpIDValue'=>$idOrden, 'clientFolio'=>$folioCliente);
		$respuesta = $client->call("checkOrder", $login);

		if ($client->getError())
		{
			echo "<br/><br/>Error al llamar el metodo ceckorder<br/>".$client->getError();
		}else
		{
			return $respuesta;
		}
